...
mod_statusbar.create{
   ...
   -- windows and icons.
   --
   -- template="%date| %workspace_name| %winname| %mpd | %workspace_num_name_pager| %filler | %exec_remote| %systray | %sysmon | %netmon",
   template="%date| %wsname_full| %winname|  %filler | %laptopstatus_cpuspeed | %laptopstatus_temperature | %laptopstatus_batterypercent | %laptopstatus_batterytimeleft | %laptopstatus_batterydrain | %exec_remote|  %netmon | %inetaddr| %systray_stalone | %systray_trayer | %systray_polybar | %systray_docker | %systray_tint2"
